...
fun <T, K> LoadingContent(
    ...
    val state: LoadingState<T> by rememberRetryableLoadingState(loader = loader, retryKey = retryKey)
...
fun <T : Any> LazyListScope.loadingList(
    ...
    retry: () -> Unit,
    key: ((T) -> Any)?,
    loading: @Composable () -> Unit = { Box { DefaultLoading() } },
    failure: @Composable (error: Throwable) -> Unit = {
        DefaultFailure(retry = retry)
    },
...
fun <T> rememberRetryableLoadingState(
    ...
    val loadingState: State<LoadingState<T>> =
        rememberRetryableLoadingState(retryKey = retryKey, loader = loader, initialValue = initialValue)
...
fun <T, K> rememberRetryableLoadingState(
    ...
    LaunchedEffect(retryKey) {
        if (!initialValue.isSuccess) {
            loadingState.value = initialValue
